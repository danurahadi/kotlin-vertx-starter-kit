...
import com.starter.app.domain.auth.handler.ApiKeyHandler
...
/**
 * Router class for API Keys resources
 *
...
class ApiKeyApiController @Inject constructor(
    ...
    post("/").jsonAsyncHandler {
        securityService.isUserAuthorized(userRole = this.get("roleName"), accessName = "api-keys.create")
        apiKeyHandler.createApiKey(this)
    }
    get("/").jsonAsyncHandler {
        securityService.isUserAuthorized(userRole = this.get("roleName"), accessName = "api-keys.list")
        apiKeyHandler.getApiKeyList(this)
    }
    delete("/:id").jsonAsyncHandler {
        securityService.isUserAuthorized(userRole = this.get("roleName"), accessName = "api-keys.delete")
        apiKeyHandler.deleteApiKey(this)
    }
})
